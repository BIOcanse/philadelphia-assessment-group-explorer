...
import { allowWorkbenchLinks,groupLinkHints } from './report_group_links.mjs';
...
const packaged=allowWorkbenchLinks(readPackagedReaderRuntime().html);
...
const hints=groupLinkHints(JSON.parse(readFileSync(inputPath,'utf8')));
const runtimeHtml=packaged.slice(0,headEnd)+fix+hints+packaged.slice(headEnd);
